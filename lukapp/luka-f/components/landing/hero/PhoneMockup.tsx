  ...
  return (
    <div className="flex flex-col gap-2 h-full">
      <div className="flex justify-between items-center">
        ...
        <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#baea0f] to-[#5913ef] flex items-center justify-center text-[10px] font-bold text-black">
          S
        </div>
      </div>
      ...
      <div className="rounded-2xl p-3 bg-gradient-to-br from-[#1A0040] to-[#0D0025] border border-purple-brand/30 shadow-[0_8px_24px_rgba(89,19,239,0.2)]">
        ...
        <p className="font-mono text-[24px] font-extrabold leading-none text-white mb-2.5 tracking-tight">$4.250.000</p>
        <div className="flex gap-3">
          ...
          <div className="ml-auto flex items-end gap-[3px]">
            {[30, 50, 38, 65, 45, 72, 60].map((h, i) => (
              <div key={i} className="w-[4px] rounded-sm bg-[#baea0f]/60" style={{ height: `${h * 0.22}px` }} />
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl p-2.5 bg-[#baea0f]/10 border border-[#baea0f]/20">
        <div className="flex gap-2 items-start">
          ...
          <p className="text-[9.5px] leading-tight text-white/70">
            Podrías ahorrar <span className="text-[#baea0f] font-semibold">$200K</span> este mes optimizando tus
            suscripciones.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-0">
        {[
          ...
          <div
            key={tx.name}
            className="flex justify-between items-center py-1.5 border-b last:border-0 border-white/[0.05]"
          >
      ...
      <div className="flex flex-col gap-2 flex-1">
        ...
        <div className="self-start max-w-[88%] bg-white/[0.07] border border-white/10 rounded-xl rounded-bl-sm px-2.5 py-1.5">
          <p className="text-[9.5px] leading-tight text-white/75">
            Llevas <span className="text-[#baea0f] font-semibold">$1.2M</span> ahorrados este mes — un{" "}
            <span className="text-[#baea0f] font-semibold">23%</span> más que abril pasado. 🎉
          </p>
        </div>
        <div className="self-start max-w-[88%] bg-white/[0.07] border border-white/10 rounded-xl rounded-bl-sm px-2.5 py-1.5">
          <p className="text-[9.5px] leading-tight text-white/75">
            Si mantienes el ritmo, proyecto <span className="text-[#baea0f] font-semibold">$3.6M</span> en 90 días.
            Reduce Rappi y llegas antes.
          </p>
        </div>
        <div className="self-end max-w-[80%] bg-[#5913ef]/40 border border-purple-brand/25 rounded-xl rounded-br-sm px-2.5 py-1.5">
        ...
        <div className="self-start max-w-[88%] bg-white/[0.07] border border-white/10 rounded-xl rounded-bl-sm px-2.5 py-1.5">
          <p className="text-[9.5px] leading-tight text-white/75">
            Usaste el <span className="text-pink-400 font-semibold">78%</span> de tu presupuesto de comida. Te quedan{" "}
            <span className="text-white font-semibold">$44K</span> para los últimos 8 días.
          </p>
        </div>
      </div>
...
function VoiceScreen({ staticWave }: { staticWave: boolean }) {
  const heights = [3, 6, 10, 7, 14, 9, 5, 12, 8, 4, 11, 7, 3, 9, 6];
  return (
    <div className="flex flex-col items-center justify-center gap-3 h-full">
      ...
      <div className="relative flex items-center justify-center">
        ...
        <div className="w-14 h-14 rounded-full bg-gradient-to-br from-[#5913ef] to-[#7a3ff5] flex items-center justify-center max-md:shadow-[0_0_16px_rgba(89,19,239,0.35)] md:shadow-[0_0_24px_rgba(89,19,239,0.5)]">
        ...
        {heights.map((h, i) =>
          staticWave ? (
            <div
              ...
              style={{ height: `${Math.round(h * 1.2)}px` }}
            />
          ) : (
            <motion.div
              key={i}
      ...
      <button type="button" className="w-full py-2 rounded-xl bg-[#baea0f] text-black text-[11px] font-bold">
        Confirmar registro ✓
      </button>
    </div>
  );
...
function ScreenContent({ screen, staticWave }: { screen: Screen; staticWave: boolean }) {
  if (screen === "dashboard") return <DashboardScreen />;
  if (screen === "ai") return <AiScreen />;
  return <VoiceScreen staticWave={staticWave} />;
}
...
export default function PhoneMockup() {
  ...
  return (
    <div className="relative w-[240px] mx-auto">
      <div className="hidden md:block absolute inset-0 rounded-[40px] bg-purple-brand/18 blur-2xl scale-110 -z-10" />

      <div className="w-[240px] h-[500px] bg-[#0A0008] rounded-[38px] border border-white/10 overflow-hidden max-md:shadow-[0_16px_40px_rgba(0,0,0,0.35)] md:shadow-[0_28px_56px_rgba(0,0,0,0.55),0_0_0_1px_rgba(255,255,255,0.05)]">
        ...
        <div className="flex flex-col h-[calc(100%-32px)] bg-[#08000F]">
          <div className="flex-1 px-3.5 pt-3 pb-1 overflow-hidden">
            {reduce ? (
              <div key={screen} className="h-full">
                <ScreenContent screen={screen} staticWave />
              </div>
            ) : (
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  className="h-full"
                >
                  <ScreenContent screen={screen} staticWave={false} />
                </motion.div>
              </AnimatePresence>
            )}
          </div>
          ...
          <div className="h-12 border-t border-white/[0.05] bg-[#050010]/80 flex items-center justify-around px-4">
            {tabs.map((t) => {
              ...
              return (
                <button key={t.id} type="button" onClick={() => setScreen(t.id)} className="flex flex-col items-center gap-0.5">
                  <t.icon
                    size={14}
                    className={active ? "text-[#baea0f]" : "text-white/25"}
                    strokeWidth={active ? 2.5 : 2}
                  />
                  <span className={`text-[8px] font-medium ${active ? "text-[#baea0f]" : "text-white/25"}`}>{t.label}</span>
                </button>
              );
            })}
          </div>
      ...
      <div className="hidden md:flex absolute top-10 -right-12 bg-[#0f0f0f] border border-[#baea0f]/25 rounded-xl px-3 py-2 text-[11px] font-semibold text-[#baea0f] whitespace-nowrap animate-float-delayed shadow-lg items-center gap-1.5">
        <Sparkles className="w-3 h-3" strokeWidth={2.5} />
        IA activa
      </div>
      <div className="hidden md:flex absolute bottom-24 -left-16 bg-[#0f0f0f] border border-purple-brand/30 rounded-xl px-3 py-2 text-[11px] font-semibold text-[#a07af8] whitespace-nowrap animate-float shadow-lg items-center gap-1.5">
        <TrendingUp className="w-3 h-3" strokeWidth={2.5} />
        +23% ahorro
      </div>
      <div className="hidden md:flex absolute bottom-44 -right-14 bg-[#0f0f0f] border border-white/10 rounded-xl px-3 py-2 text-[11px] font-semibold text-white/60 whitespace-nowrap shadow-lg items-center gap-1.5">
